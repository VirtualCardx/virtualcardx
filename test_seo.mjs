...
const checks={p2_status:p2.status,p2_h1:(p2.text.match(/<h1\b/g)||[]).length,p2_title:(p2.text.match(/<title>(.*?)<\/title>/)||[])[1],p2_has_about_card:/<h2 class="post-title"><a[^>]+>About<\/a>/.test(p2.text),p18_status:p18.status,p19_status:p19.status,sitemap_status:sm.status,sitemap_count:urls.length,sitemap_unique:new Set(urls).size,sitemap_has_roots:urls.includes('https://virtualcardx.com/')&&urls.includes('https://virtualcardx.com/en/'),sitemap_has_zh_about:urls.includes('https://virtualcardx.com/about/'),security:p2.headers['x-content-type-options'],csp:!!p2.headers['content-security-policy'],favicon_status:fav.status,favicon_location:fav.headers.location};
console.log(JSON.stringify(checks,null,2));
if(checks.p2_status!==200||checks.p2_h1!==1||checks.p2_has_about_card||checks.p18_status!==200||checks.p19_status!==404||checks.sitemap_count!==374||checks.sitemap_unique!==374||!checks.sitemap_has_roots||!checks.sitemap_has_zh_about||checks.security!=='nosniff'||!checks.csp||checks.favicon_status!==301)process.exit(1);
